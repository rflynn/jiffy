...
scale(R, S, MPlus, MMinus, LowOk, HighOk, Float) ->
    io:format("R S: ~p ~p~n", [R, S]),
    Est = int_ceil(math:log10(abs(Float)) - 1.0e-10),
    io:format("EST: ~p ~p~n", [math:log10(abs(Float)) - 1.0e-10, Est]),
...
generate(R0, S, MPlus, MMinus, LowOk, HighOk) ->
    D = R0 div S,
    R = R0 rem S,
    io:format("R0 S D R MP MM: ~p ~p ~p ~p ~p ~p~n", [R0, S, D, R, MPlus, MMinus]),
    TC1 = case LowOk of
              true ->
                  R =< MMinus;
              false ->
